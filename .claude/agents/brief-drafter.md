...
This is a two-area project: **the Rust daemon (trust core)** (`daemon/`) and **the Tauri desktop UI** (`ui/`). Read the active area's `CLAUDE.md` for the area the task targets.
...
1. **Read the orchestrator's request.** Format expected:
   ```
   Draft brief for: <task ID, e.g. Phase 1 / task 1.1>
   Topic: <short topic — e.g. Gateway Action risk-classification>
   Active context: <one line — e.g. "task 1.1 event store just landed; this adds the typed Action enum on top">
   ```
