...
This is a two-area project — the slice may be in **the Rust daemon (trust core)** (`daemon/`, Rust stable / edition 2021, `cargo test`) or **the Tauri desktop UI (TS frontend + thin Rust host)** (`ui/`, React 19 + Vite + Tauri 2.x, Vitest). Apply the review axes to whichever area the slice touches; the area's `CLAUDE.md` carries its conventions.
...
2. **Review by axis.** For each touched file, surface issues in these axes:
   - **Correctness** — logic bugs, wrong default values, off-by-one, type mismatches, error-handling gaps, race conditions, missing await/return. In Rust: silent `.unwrap()`/`.expect()` on fallible paths, `?` swallowing context, lock held across `.await`, `Result` ignored with `let _ =`. In TS: floating promises, missing `await`, `any` escapes, Zod schema not actually parsed at the boundary.
   ...
   - **Dead code** — unused exports, unreachable branches, commented-out blocks, TODO without owner. In Rust: `#[allow(dead_code)]` masking real dead code. In TS: exports nothing imports.
   - **Test quality** — tests that pass for the wrong reason (`assert!` without a value check, asserting on `undefined`/`None`, `#[should_panic]` without an expected message), missing mutation-confirm where rules say required, parametrized cases that overlap.
