...
You routed each Step-9 item hot during the session, per the **canonical matrix in `docs/orchestrator-briefing.md`** (loaded at `/orchestrate-start` — don't re-copy it here). Verify each landed: grep the lesson title in `daemon/LESSONS.md` (or `ui/LESSONS.md` for the Tauri UI area) + its linked index row in `daemon/CLAUDE.md` (or `ui/CLAUDE.md`); `git diff` any `ARCHITECTURE.md` edits; grep Carry-forward / the phase for routed TODOs; confirm any deferment was escalated. The **most-likely-to-slip** is *Completed work → ticked checkbox* — Step 3.
...
> For a UI-area round, stage `ui/LESSONS.md` + `ui/CLAUDE.md` instead of (or in addition to) the daemon-area lesson/convention files.
...
Then **push to origin (git@github.com:SiWarlock/NexusOps.git) only**:
