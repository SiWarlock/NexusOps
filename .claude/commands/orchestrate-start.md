...
You're starting an orchestrator session for NexusOps. Orient before taking any action.
...
(b) **The active area's cross-doc invariants table has changed** since the prior brief touched it (check via `git log --oneline daemon/CLAUDE.md` for the Rust daemon area, or `git log --oneline ui/CLAUDE.md` for the Tauri UI area).
