...
- the **phase set** this track owns (a subset of `Phase <N> (tasks <N>.<M>)`),
- its **code area(s)**,
- its **upstream-track dependencies** (tracks that must merge before this one integrates),
- its **worktree path + branch** (default convention: `../NexusOps-<track>` on branch `track/<track>`).
...
1. **`IMPLEMENTATION_PLAN.md`** — focused sections:
   ...
   - **The active phase within this track's phase set** (Phase <N> (tasks <N>.<M>, e.g. Phase 1 / task 1.1) prefix) — task IDs + one-line topics. (Single-track plan → the active phase globally.)
...
```bash
git worktree add ../NexusOps-<track> track/<track>   # or the base branch the Track map specifies
```
...
```
You are <track>-<area>-orchestrator on the NexusOps agent team.
...
```
You are <track>-<area>-implementer on the NexusOps agent team.
