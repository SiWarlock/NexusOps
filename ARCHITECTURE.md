...
| **EventTypeRegistry** (MVP subset) | §7.1 | per event_type: event_version, payload schema, sensitivity default, consuming projection(s). **First entry `SessionStarted` implemented 1.2 (`shared/src/events.rs`; CONTRACT_VERSION → 0.8.0; 3-way verify @ 0.8.0). Accretes per phase as event types land.** |
