...
Ten machines (the 8 of the `/arch-draft` rough draft + **ActionRequest** [R-5] + **AgentTeam** promoted to first-class [R-6]). Nine are frozen in `shared/` (0.5); **ExecutionProfile** — the 10th — is held for **0.5b** (§5.0; Appendix A). Enums are binding; each declares terminal states; illegal transitions → degraded marker (R-9, §17). Stored as `TEXT status` on the named projection/registry row.
