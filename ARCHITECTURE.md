...
| **ActionResult** | §6.2 | **Frozen 2.1a.** action_request_id, status (`ActionResultStatus`: succeeded\|failed\|partially_succeeded\|cancelled), created_resources[], changed_resources[], emitted_events(Vec\<EventId\>), error?(Option\<String\> — **structured `ActionError`→2.4**), rollback_available. |
