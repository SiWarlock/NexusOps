...
| **GatewayPort** | §6.1/§6.4 | submit_action, submit_action_plan, preview_action, approve, deny, get_projection, subscribe, get_capabilities. **[IMPLEMENTED 1.5] READ surface live** (get_projection/subscribe/get_capabilities) over the UDS transport (4-byte-len+JSON framing, `getpeereid` peer-auth, HelloFrame/HelloAck/VersionSkewError handshake, `ServerFrame` frame-mux, `protocol_error`); **mutation methods (submit_*/approve/deny/preview) → Phase 2.** Cross-language IPC contract @ **CONTRACT_VERSION 0.11.0**; `protocol_version` `{min:1,max:1}`. |
