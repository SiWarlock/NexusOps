...
**UI TRACK (`track/ui`) — Phase 6 round 2 SEALED (2026-06-08).** ✅ **6.1** (`fd9738b`/`39a87c6`/`402f4c5`) · ✅ **6.2** (`b32c3c0`/`e2cebbc`) · ⏳ **6.3 PARTIAL** — 6.3a (`144b6b6`) + 6.3b (`c420cd7`+`885cc0d`) + 6.3c (`23fbda3`) landed; **6.3d/6.3e ⏸️ PARKED on daemon-1.5** (Decision C) · ⏳ **6.4 PARTIAL** — 6.4a (`f70757e`) + 6.4b (`db9b89b`) + 6.4c (`765923f`) + 6.4d (`290381a`) + 6.4e (`823d16e`) landed; **6.4d-2 (§17 safety-state) + checking-banner remain** · ⏳ **6.5 theme/visual pass NOT STARTED** (the unstyled-layer Finding — dedicated end-of-logic pass + automated visual gate). **116 tests green** (`347c31f..823d16e`), tsc/oxlint/vite-build clean; built against frozen `shared/` 0.5.0 + `MockGatewayPort` + fixtures + `NexusOps-ui-kit`. Lessons banked this round: `ui/LESSONS.md §7`/`§8`/`§9`/`§10` + a `§6` refinement. **Next ui session target (FRESH team): 6.4d-2 (§17 safety-state display — security-reviewer) + checking-banner → THEN the 6.5 theme pass.** Round-2 terminal commit: _(this `/orchestrate-end` commit)_; pushed to `origin track/ui`. Session docs: `ui-001` (round 1) + `ui-002-2026-06-08-...` (round 2, `731c355`).
...
_(Triaged at the 2026-06-08 **round-2** ui close-out — **5 items**, under the ~7 cap. **DELETED (consumed):** the 6.3 `data-item-id` locator convention + the shared-`SessionItem` extraction — both landed in **P6.3b L1** → banked `ui/LESSONS.md §8`. Phase-scoped 6.4 follow-ups (zero-projects graph state, sessions-table filtering, tablist roving + §9-audit refinement, sidebar resume-mode indicator, TopBar back/forward history-nav) were **INLINED** as `- [ ]` tasks under §6.3/§6.4. The synthetic-rank `compareByAttention` narrow was **dropped** (no synthetic-rank caller appeared across 6.3). Remaining = 1 perf-at-scale nit + 4 cross-track spreads. **Next ui slice = 6.4d-2 (§17 safety-state display).**)_
...
- [ ] **Graph zero-projects degraded state** — the Project Graph builds a `project:` root from `data.projects[0]?.project_id ?? ""` (empty-pid/empty-label root) when the app has no projects (untested, uncommon first-run path). Guard with an explicit no-projects state; resolves with real **project-selection** (the deferred Q3 graph project-source → ProjectSwitcher selection). _(origin: 2026-06-07 P6.3b; consumer: 6.3c / Phase 7 project-selection)_
...
### 6.5 — Graphite Arc theme/visual layer + automated visual gate — ⏳ DEDICATED PASS (end of Phase-6 logic; human-sequenced 2026-06-07)
...
- [ ] **Theme/base layer** — a global app stylesheet applying the kit **semantic** tokens: `body`/`:root` (dark `--surface-*` bg + Geist `font-family` + base text), the cockpit **layout grid** (top-bar / sidebar / content / right-drawer / activity-dock), panel/card **chrome** (surfaces/borders/`--r-*` radii/elevation). Reference the semantic layer only (never primitives — the re-hue promise, §11.1).
- [ ] **Theme the shell + views** — TopBar/Sidebar/DrawerStack/ActivityDock/StatusBar + the content views (CC/Graph/Sessions/Usage), replacing the bare structural inline styles with token-referencing classes. **Separate "built-but-unstyled" (TopBar/Sidebar/etc → style)** from **"not-yet-built / parked-with-the-intent-seam" (right-hand Human Input Queue → a surface to BUILD, partly intent-coupled — may stay parked).**
- [ ] **Automated visual gate (acceptance mechanism + standing gate)** — a gstack-browser comparison of **(a) the running dev server** vs **(b) the runnable Graphite Arc reference `NexusOps-ui-kit/ui_kits/control-plane/index.html`** (CONFIRMED to exist — the assembled cockpit prototype), verifying theme + styling MATCH. Built via `/browse` · `/design-review` · `/qa` + the chrome browser tools; becomes the **standing visual-verification gate** for UI work going forward (run at phase boundaries + when a styling slice lands). _(origin: 2026-06-07 theme-finding / human decision)_
- [ ] **Acceptance = VISUAL** (not "render test green") — the dev server visually matches the Graphite Arc prototype (dark cockpit, Geist, layout, chrome). Optional lightweight wiring-guard (assert the theme stylesheet is imported + applies `body` styling) catches gross absence in unit tests but does NOT replace the rendered comparison.
...
- [ ] **Graphite Arc theme/visual layer applied (6.5) + the automated visual gate passes** — the running dev server visually matches `NexusOps-ui-kit/ui_kits/control-plane/index.html` (not implied by green unit tests — `ui/LESSONS.md §10`).
