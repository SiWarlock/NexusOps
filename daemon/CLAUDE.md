...
| Event-type registry (`EventTypeRegistry`) | §7.1 / Appendix A | **`SessionStarted` = first entry (1.2, `shared/src/events.rs`); CONTRACT_VERSION → 0.8.0. `DeviceRegistered` + `LocalRunnerRegistered` added 1.6a-L3** (System-actor cold-start registration via the write-actor; `actor_type=System`; reserved `SYSTEM_WORKSPACE_ID` sentinel); **CONTRACT_VERSION → 0.12.0**. **`AuditIntegrityViolation`{seq,reason} added 1.6c** (§17 degradable-replay quarantine record; System-actor; **CONTRACT_VERSION → 0.13.0**; 3-way verify @ 0.13.0). Per-type payload schema, contract-first per §5.0 (schemars → schema → Zod/Pydantic); the golden-log tests bind. Accretes per phase as event types land. |
