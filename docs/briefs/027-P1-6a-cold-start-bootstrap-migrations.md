...
- [ ] **Ids:** add `DeviceId`/`LocalRunnerId` newtypes via a **NEW `desktop_minted_id!` macro** (a sibling to `minted_id!`) in `shared/src/objects.rs`, keyed off `DesktopObjectKind` with the prefix from `DesktopObjectKind::id_prefix()` (`dev_`/`lr_`) and reusing `ids::IdError` — **NOT** the existing `minted_id!` (which is keyed off the frozen-22 `IdKind`, so it can't mint these without expanding the set) and **NOT** new `IdKind` variants (the frozen-22 stays untouched). **Frozen-22 guard test:** assert `IdKind::from_prefix("dev_") == None` / `"lr_" == None`. (USER-RULED Option B; blessed Step-2.5 design in `docs/sessions/006-2026-06-08-cold-start-bootstrap-and-runtime.md` §"1.6a-L3 build spec" lines 84–110.)
...
5. Step 2.5 — L1+L2 ✅ landed (`48b7a2c`/`f1de088`). **L3 UNBLOCKED (USER-RULED Option B) — send the L3 test-design write-up when you take it up.** **Build spec = the blessed Step-2.5 design in `docs/sessions/006-2026-06-08-cold-start-bootstrap-and-runtime.md` lines 84–110** (the `desktop_minted_id!` macro + `SYSTEM_WORKSPACE_ID` sentinel + `first_event_of_type` + the 5-test set; the RED tests were drafted then reverted at close-out — recreate them from item 6 there).
