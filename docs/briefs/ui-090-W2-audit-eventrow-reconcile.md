...
- [ ] The local `ACTOR_LABEL` map is **REPLACED/extended, keyed on the REAL 10 `ActorType` wire values** (`user`/`project_brain`/`action_gateway`/`workflow_runtime`/`local_runner`/`session_adapter`/`integration_syncer`/`system`/`remote_client`/`automation_policy`), **tolerant of an unknown key → raw-string fallback**, UNBOUND from the `ActorType` Zod enum (Q1 RESOLVED+CORRECTED — the earlier `human/agent/brain/pack` example values were phantom); no reference to the removed `actor_type` field remains (tsc-enforced).
...
6. **`actor_label_maps_known_and_falls_back_on_unknown`** — a KNOWN actor wire value (`action_gateway`) renders humanized ("Gateway"); an UNKNOWN value (e.g. `future_actor`, outside the 10 `ActorType` wire values) renders the RAW string (forward-compat, never fail-closed / never enum-rejected). Why: the daemon snake_case `ActorType` wire value + the UI-side tolerant map (Q1 RESOLVED — the load-bearing test the phantom values would have broken).
...
1. **`actor_label` rendering — RESOLVED + CORRECTED (verified against the sealed `9f7e396`).** `actor_label` = `wire_value(&env.actor_type)` (`daemon/src/projections/audit.rs:28`) → the **snake_case wire value of the audit `ActorType` enum** (`shared/src/actor.rs`): `user`/`project_brain`/`action_gateway`/`workflow_runtime`/`local_runner`/`session_adapter`/`integration_syncer`/`system`/`remote_client`/`automation_policy`. NOT humanized; **NOT a different enum** — an earlier relay's `human/agent/brain/pack` example values were **phantom** (caught at Step-2.5; verified against the sealed code). The VALUES are **unchanged** from the old `actor_type` field; only the field NAME changed + it's now `Option<String>` served as a plain string. **Decision:** `actor_label` = `z.string().nullable()`, **UNBOUND from the `ActorType` Zod enum** (forward-compat — a future daemon-added actor renders raw, never fail-closes the tile); **REPLACE/extend the `ACTOR_LABEL` map keyed on the REAL 10 wire values** (`user`→"You", `project_brain`→"Brain", `action_gateway`→"Gateway", `workflow_runtime`→"Workflow", `local_runner`→"Runner", `session_adapter`→"Adapter", `integration_syncer`→"Integration", `system`→"System", `remote_client`→"Remote Client", `automation_policy`→"Automation"), **tolerant of an unknown key → raw-string fallback**. **NO completeness-pin to `ActorType`** (the raw fallback IS the forward-compat). The per-actor cosmetic styling re-keys to the real values (`user`→accent, `project_brain`→brain-ink) or drops (cosmetic; implementer's call).
