...
- [ ] The local `ACTOR_LABEL` enum-map + the `actor_type === "user"|"project_brain"` enum-equality styling are reconciled (see Step-2.5 Q1) — no reference to the removed `actor_type` field remains (tsc-enforced).
...
6. **`unknown_actor_label_is_not_enum_validated`** — a row with an `actor_label` outside the old `ActorType` set still renders (forward-compat). Why: the daemon plain-string/forward-compat note.
...
1. **`actor_label` rendering — drop the local humanization map?** The daemon now serves `actor_label` as a wire-string render (NOT the `ActorType` enum), so the local `ACTOR_LABEL` map + the `actor_type === "user"|"project_brain"` enum-equality styling (accent/brain-ink) no longer apply by construction. Default vote: **render `actor_label` directly (`e.actor_label ?? "System"`), DROP the enum-map + the per-actor cosmetic styling** (uniform actor rendering) — the daemon owns the label; the UI must not re-derive actor identity from a free-form string. _(I'll confirm with the daemon-orch whether `actor_label` is humanized ("Gateway") or snake_case ("action_gateway"); if snake_case, a thin label-keyed humanization map MAY stay — but still unbound from the enum.)_
