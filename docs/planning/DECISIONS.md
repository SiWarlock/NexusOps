...
### Decision
Define the Gateway as a **`GatewayPort` interface**; implement execution **inside the daemon** (the single SQLite writer). Transport between UI/clients and the daemon is a **Unix domain socket** (OS-enforced peer auth; no exposed TCP port — best for the local trust boundary), carrying **4-byte length-prefixed JSON-RPC**. **[RECONCILED 2026-06-07 — §6.4/§15]** macOS peer-auth = **`getpeereid()`** (`SO_PEERCRED` is Linux-only); filesystem perms (0700 dir / 0600 node) are defense-in-depth, not the primary control. Framing is **length-prefixed only** — newline-framing was dropped (§6.4). Agents and the Brain submit **intents** that flow *into* the Gateway; they never mutate directly (preserves the single chokepoint). MCP tool calls from agents/Brain are *producers of intents*, not the Gateway transport itself.
...
### Decision
- **Dual git backend:** `git2-rs` (libgit2) for **hot structured reads** (status/diff/log/branch/worktree-list); the **git CLI for ALL mutations** (worktree add/remove, branch, commit, merge, checkout) and libgit2-gap cases. **[RECONCILED 2026-06-07 — OQ-INT-SPIKE-6 → ARCHITECTURE.md §9]** libgit2 **≥ 1.9.4 (git2 0.21) CAN fully read `extensions.relativeWorktrees` repos** (open/statuses/branches/head/worktree-list/find_worktree/diff) — the earlier "git ≥2.48; fix unreleased" premise is **superseded**; relative-worktree repos no longer force CLI reads. libgit2 still **misreports sparse-checkout** — that (separate, still-unverified) gap is now the *only* retained CLI-read fallback. Since NexusOps runs git in embedded PTYs and agents make their own worktrees, libgit2 disagreeing with the user's terminal is a credibility bug. **Mutations remain CLI-only regardless** = one Action-Gateway chokepoint + terminal parity (the chokepoint invariant, not a libgit2 limitation).
...
### What Would Change This
octocrab merge/GitHub-App ergonomics proving inadequate (spot-check owed). _(The earlier "libgit2 shipping relative-worktree support" trigger **has now occurred** — libgit2 ≥ 1.9.4 reads relative worktrees, verified by OQ-INT-SPIKE-6 and folded into §9; it widened git2's read scope as predicted. Mutations stay CLI-only.)_
