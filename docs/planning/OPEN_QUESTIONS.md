...
| **OQ-HARN-SPIKE-2** | **Human-interactive-PTY vs SDK-driven handoff** model for Claude Code — when does the human "own" a session vs the Action Gateway, without double-driving the harness. _(Carried into the finalized artifacts as **OQ-HARN-SPIKE-7** — gap-audits, ui-review, MVP_TASKS 0.1; executed 2026-06-07 → `docs/spikes/OQ-HARN-SPIKE-7.md`. Canonical ID going forward = **-7**.)_ | **finalize** | ADR-006, dep #2 |
...
| **OQ-DATA-SPIKE-5** | **[RESOLVED 2026-06-07 — 0.5 freeze (06f9576); §5.3 + `shared/src/objects.rs`]** Missing desktop objects — `Device`, `RemoteClient`, `LocalRunner`, `EventProjection`: defined in `DATA_MODEL §6`, frozen in `shared/` (prefixes `dev_`/`rc_`/`lr_`/`eprj_`); LocalRunner+EventProjection MVP-live, Device+RemoteClient deferred (iOS). | ~~finalize~~ ✅ done | ADR-005-adjacent, dep #5 |
| **OQ-INT-SPIKE-6** | **[PARTIAL 2026-06-07]** libgit2 relative-worktree **read-path RESOLVED** (git2 0.21 / libgit2 ≥ 1.9.4 CAN read `extensions.relativeWorktrees`, §9 + `OQ-INT-SPIKE-6.md`); **octocrab `pulls().merge()` / GitHub-App spot-check still owed**. Called out in ADR-007 "What Would Change This". | **pre-build spike** | ADR-007 |
...
| OQ-DATA-6 | Missing desktop objects — `Device`/`RemoteClient`/`LocalRunner`/`EventProjection`? | `DESKTOP_FIRST_RUNTIME §7` requires them; `SHARED_OBJECT_MODEL` lacks all but Terminal. | Define in `DATA_MODEL.md` during finalize. | **finalize** | Stub minimal fields for MVP (only `LocalRunner` likely needed pre-iOS). | **✅ RESOLVED** — 0.5 freeze (06f9576), §5.3 + `shared/` |
| OQ-DATA-7 | Task vs PlanTask — separate objects or subtype? | `SHARED_OBJECT_MODEL §37.2`; affects schema + linking. | PlanTask as a specialized Task (lean toward one base). | MVP-blocking | Two separate objects. | **✅ RESOLVED** — R-8 superset (one `tasks` table, kind-scoped), §5.1 / ADR-012 |
...
| OQ-INT-9 | octocrab merge / GitHub-App ergonomics + libgit2 relative-worktree gap re-check? | ADR-007 "What Would Change This." | Spot-check octocrab merge; re-check `extensions.relativeworktrees` (git ≥2.48). | **pre-build spike** | shell `gh` for merge; CLI-only worktrees. | **PARTIAL** — libgit2 read-path RESOLVED (§9, OQ-INT-SPIKE-6); octocrab-merge spot-check open |
