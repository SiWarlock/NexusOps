...
> **Orchestrator-side Round-3 round doc** (companion to the implementer's `edges-005-2026-06-12-r3-linear-network-adapter.md`). Predecessor: `edges-004` (R2 orch) ← `edges-003` (R2 impl) ← `edges-002` (R1 orch) ← `edges-001` (R1 impl). Successor: _(Round-4 — TBD, post-restart)_. **Multi-track:** this doc carries the **PLAN-DELTA HAND-OFF** for the integration owner — the shared `IMPLEMENTATION_PLAN.md` / `ARCHITECTURE.md` / `daemon/LESSONS.md` / `daemon/CLAUDE.md` are integration-owned and **NOT edited from `track/edges`**; apply this at the P5/P7.1 phase-exit merge. Lead decisions live in `docs/team-handoffs/edges-lead-decision-log.md` (referenced, not duplicated).
