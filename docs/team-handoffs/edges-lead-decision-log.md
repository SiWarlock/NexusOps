...
## Round 6 (post-cycle — P7.1 Wave-D) — away/automated mode · 2026-06-13

Fresh pair (orch + impl) spawned post-R5-cycle from the edges-R5-wiring-plan PLAN-DELTA; both read-backs clean (correct start cmds, registries, edges- prefixes). R6 = P7.1 Wave-D (github/linear sync executors → `PullRequestSynced`/`{Github,Linear}SyncFailed` + `proj_pull_request` projector) + the phase-exit tail. Opener = edges-023 github sync executor.

**3a MECHANISM CORRECTED (orch trace, evidence-backed — lead DEFERRED to the evidence):** the carried 3a note ("spawn_blocking + Handle::block_on; block_on-on-a-worker-PANICS") was refined by the as-built — `execute()` runs on the write-actor's **RAW std::thread** (writer.rs:273-326 → pipeline.rs:969): NOT a tokio worker, NO entered runtime → `Handle::current()` PANICS / `spawn_blocking` is awkward there (the OPPOSITE footgun). **CORRECT mechanism = capture a `tokio::runtime::Handle` in main.rs's #[tokio::main] runtime → inject into the executor → `handle.block_on(async{…})`** (block_on from a non-runtime thread does NOT panic; reactor runs on the runtime workers). NO spawn_blocking. → folds into the Wave-D LESSON; impl pins it RED-first at Step-2.5.
