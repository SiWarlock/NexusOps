...
- **Context trajectory R2 (max=impl):** 35%(2)→42%(3)→50%(4), ~+8%/slice. Next cycle ~2-3 slices out (WARN ~70%). Lead steady ~31%. **Cycle plan: apply the R1 lesson — set the cycle policy + let the ORCH pick the clean stopping point (ideally a vertical-complete boundary intersecting WARN); don't micromanage slice-sequencing.**
