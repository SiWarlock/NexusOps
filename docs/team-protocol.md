...
Naming: **`<track>-<area>-<role>`** when parallel teams run (e.g. `frontend-team-orchestrator`), else `<area>-<role>` (e.g. `daemon-orchestrator`) — full rule in root `CLAUDE.md` "Naming + cross-bleed prevention."
