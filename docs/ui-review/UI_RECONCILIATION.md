...
- **One canonical status→attention-rank table** covering every state of all 9 machines; sidebar weight + queue membership + sort order all derive from it. **No silent fall-through to idle** (today `waiting_on_permission`, conflict, stale, blocked, changes_ready all floor to rank 0 and never enter "Needs my attention"). Order per PRD §5.2.
