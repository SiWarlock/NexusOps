...
|---|---|---|
| Generated Zod contract layer (`ui/src/contracts/generated.ts` + `index.ts`) | §5.0, §5.1, §6.4 | Generated, drift-caught consumer of the frozen `shared/contracts/schema/nexusops-contract.schema.json`. **33 value-sets at `CONTRACT_VERSION 0.19.0`** (the P6.2 0.19.0 regen; was 20 at 0.12.0 — +13 Phase-2 Gateway enums). The two Gateway-status `$def` **renames** `ActionRequest`→`ActionRequestStatus` / `Approval`→`ApprovalStatus` are the schema names; the UI status-machine identifiers (`"ActionRequest"`/`"Approval"`) stay **UI-render-policy** (A-lite — decoupled from the `$def` name), bridged by a 2-entry `VALIDATOR_KEY` alias. Never hand-edited; regen via `ui/scripts/gen-contracts.mjs`; **`validators` derives from the generated bundle (`= shape`), never hand-listed** (the hand-list is what drifted at 0.12.0 → [[14]]). Pinned by accept-all / reject-unknown / set+`.options` drift / `CONTRACT_VERSION`===`x-contract-version` tests (P6.1a) + the §6.4 `ServerFrame`/`WireError` frame-mux schema-snapshot (P6.2). 6.2 extends with the status→attention-rank coupling. |
