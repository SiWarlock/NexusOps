...
| 13 | 2026-06-08 | [UI selection/scope over a frozen projection](LESSONS.md#13) | A UI selection/scope (active-project; filters) is plain UI state over the FROZEN projections — NOT a mutation/provisional/Gateway-intent → no `canSubmitIntent` gate, no daemon dep (ships without the parked intent seam). Read via a context provider wrapping the whole shell (mirror `ReadOnlyProvider`); scoped views re-root/filter, cross-cutting triage (Command Center) stays GLOBAL; a stale selection re-scopes to a default (no ghost id). Distinct from the daemon-1.5 mutation path. |
