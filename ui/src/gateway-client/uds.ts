...
import { parseCapabilities, parseDiff, parseProjectionPage } from "./boundary";
import {
  canTransition,
...
export class UdsGatewayPort implements GatewayPort {
  ...
  // The streaming subscribe (§6.1 ProjectionDelta) = slice 052 (the Rust subscribe_stream
  // + the Tauri Channel + the TS AsyncIterable). Throws synchronously until wired.
  subscribe(_params: SubscribeParams): AsyncIterable<ProjectionDelta> {
    throw new Error(
      "UdsGatewayPort.subscribe: subscribe streaming is not wired (slice 052)",
    );
  }
